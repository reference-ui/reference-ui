...
test.describe('T9 — full mix (2 extends + 2 layers)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
  })

  test('renders the chain-t9 root', async ({ page }) => {
    await expect(page.getByTestId('chain-t9-root')).toBeVisible()
  })
  ...
  test('assembled @layer prelude lists extends... before layers... before local', async ({ page }) => {
    ...
    // Find any prelude that has all four upstream names plus chain-t9.
    ...
    expect(preludeText).toContain('chain-t9')

    // Verify ordering on a single prelude entry (the first that mentions chain-t9).
    const candidate = preludeText
      .split('|')
      .find(entry => entry.includes('chain-t9'))
    expect(candidate, 'expected a prelude entry containing chain-t9').toBeDefined()
    ...
    expect(indexOf('layer-library')).toBeLessThan(indexOf('chain-t9'))
    expect(indexOf('layer-library-2')).toBeLessThan(indexOf('chain-t9'))
  })
})
