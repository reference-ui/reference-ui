...
Status: `idea` — standing red-team, not a slice voyage. This file is
the operating document. Folded in and retired:
`doom-agent-protocol.md` (the protocol below *is* that file).

Protocol: ARMED — trigger: `overmatch-ledger.md` (Ph5) exists AND the
...
"satisfied" signal. Neither condition may be inferred. Overmatch and
Forge are concluded; the marker is the remaining gate.
...
- Forge decided the *shape* so Doom is not inventing architecture. Seed 1
  (nested import-spread silence) is already fortified there.
...
Overmatch and Forge close. Doom agents try to break the system, findings
are reviewed, real breaks are fortified (tests formalized + implementation
...
- `reference-lib` unit / CT when the break is a component contract.
- Full vertical stack crews: break across layers at once (authoring →
  extract → resolve → emit → paint).

Prime layer crews first, then vertical crews. Cheapest first (captain
note): RS units → Neo cases → lib unit → browser last. Many break areas
exist beyond atomic styles — crews are expected to find them.
...
- Auto-filed, never auto-fixed. Fixes ride the normal slice process
  with case proof.
...
[Operation Forge](completed/operation-forge.md) §1, and Forge Slice 3
owns the fix (module graph, residue marker, `ATM-SITE-78`), so Doom
cycle 1 reproduces against that fixed shape. **FORTIFIED 2026-09-19** —
