...
Procedure: `agentrs-counters/2` — same-run phase boundaries on span + census legs; timestamped libc event log bucketed into phases (v1 had whole-worker census minus a bare-node baseline, span from another run). Span leg profiles the release+counters-trace
...
| stall cycles (width-4 bound) | 1,660,207,422 (41%) |
...
- Compute room: IPC 2.36 → healthy — some headroom in stalls; stall share 41% against a width-4 bound.
