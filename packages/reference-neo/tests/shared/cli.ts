...
//   agentneo run [case-id]      serve world + run specs headless (all cases if omitted)
...
import { listCases, searchCases, getCase, type NeoCase } from './cases.ts';
import { runCase, type RunResult } from './runner.ts';
import { failedIds, LAST_RUN_FILE, orderFailFirst, readLastRun, writeLastRun, type LastRunEntry, type LastRunLog } from './lastRun.ts';
...
function applyRunArg(flags: RunFlags, arg: string): void {
  ...
  if (flags.id !== undefined) throw new Error('usage: agentneo run [case-id] [--failed] [--update-snapshots --confirm]');
  flags.id = arg;
}
...
function selectRunCases(flags: RunFlags, log: LastRunLog | null): NeoCase[] {
  ...
  return flags.id ? [getCase(flags.id)] : listCases();
}
...
try {
  if (cmd === 'list') code = await cmdList();
    ...
    console.error('usage: agentneo <list|search|run|q> [...]');
    code = 1;
  }
} catch (err) {
