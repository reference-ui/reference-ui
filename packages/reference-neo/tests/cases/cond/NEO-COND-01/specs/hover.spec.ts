// hover.spec.ts — spec for NEO-COND-01, the hover dual-bind case. Takes
...
// data-hover arm without interaction; the live element proves the :hover
// arm under a real hover and its release back to the base color.
export default async function run({ page, case: c }: SpecInput): Promise<void> {
  ...
  const utilityCount = styles.match(/\.neo-cond-01__/g)?.length ?? 0;
