...
| NEO-SYNC-17 | `extends` strips upstream `_private` at merge: nested secret + top-level vault gone from tokens, provenance, and types, while the upstream public token and the downstream's own `_private` paint | done | BAS-EXTEND-03/05 (TS-side mirror — the single-script path never reaches the Rust boundary) | `fragments/base` (`scopeUpstreamTokenFragment` set-gate + `stripPrivateTokensDeep`; merge + provenance consume scoped tokens) | two-system world: `#up-probe` + `#own-probe` computed colours; `evaluated-system.json` tokens/provenance pins; `.d.ts` lacks the upstream path | `tokens.ts` header contract, NEO-SYNC-10 (pins unmoved), NEO-TOKEN-09 (owner passthrough) |
