...
| NEO-PRIM-11 | Array `css` props flow through the split to `css()` merge and paint; the prop type admits arrays | done | ATM-SITE-19 (RS-23 extraction green) | `split.ts` + `generate.ts` | P15 `#p15` paints both utilities; split/factory unit tests | `[panda-v1]` `jsx.test.ts:529`; N3 R1 `/tmp/n3cook-r1.mjs`; case green + PARITY-01 green |
