...
| NEO-RESP-05 | Range conditions `mdDown`, `mdOnly`, `smToLg` bound the query and never overlap at the boundary | done | ATM-COND-13 | — | computed at boundary±1px | `[panda-v1]` `breakpoints.test.ts` epsilon; `[atm]` P1 #11; smOnly≡smToMd identical query (K3) |
