// spread.spec.ts — spec for NEO-SITE-08, the logical-spread case. Takes
...
// The const-guarded logical spread unpacks beside its sibling: the sheet
...
export default async function run({ page, case: c }: SpecInput): Promise<void> {
  ...
  assert.ok(styles.includes('.neo-site-08__c_red {'), 'sheet carries the sibling color');
  assert.ok(styles.includes('.neo-site-08__m_10px {'), 'sheet carries the spread margin');
  const utilityCount = styles.match(/\.neo-site-08__/g)?.length ?? 0;
  ...
  assert.equal(
    ...
    'neo-site-08__c_red neo-site-08__m_10px',
