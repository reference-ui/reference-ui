// type.spec.ts — spec for NEO-TYPE-03, the recipe-variant case. Takes
...
const RECIPE = `import { recipe, type RecipeVariantProps } from '@reference-ui/react'
...
// Every axis selects as an optional literal union: through the named
// RecipeVariantProps position, through a direct call, through raw() and
// splitVariantProps(), and with no selection at all. Materialized into a temp
// dir at spec time because it cannot live in the repo: the harness pre-run
// typecheck resolves @reference-ui/react to the stable surface, which has no
// RecipeVariantProps.
const POSITIVE = `${RECIPE}
type Selection = RecipeVariantProps<typeof button>
const selection: Selection = { tone: 'accent', size: 'lg' }
export const a: string = button(selection)
export const b: string = button({ tone: 'accent' })
export const c: string = button()
export const d: unknown = button.raw({ size: 'sm' })
export const e: unknown = button.splitVariantProps({ tone: 'muted' })
`;

// 'bogus' is not a declared tone, so the tightened call position rejects it
// with TS2322. Same temp-dir materialization as the positive file.
const NEGATIVE = `${RECIPE}
export const bad: string = button({ tone: 'bogus' })
`;
...
async function typecheckFile(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'neo-type-03-'));
  try {
...
// After sync, the generated declarations type recipe selections as optional
// literal unions at every call position, reject a wrong variant value with
// TS2322, carry no ConditionalValue wrap, and the same world paints its
// recipe probe.
export default async function run({ page, case: c }: SpecInput): Promise<void> {
  ...
  assert.ok(
    !reactTypes.includes('ConditionalValue'),
    'recipe variant props are plain unions, not ConditionalValue-wrapped',
  );
  ...
  assert.equal(
    positive.code,
    0,
    `valid variant selections compile at every call position:\n${capped(positive.text).join('\n')}`,
  );

  const negative = await typecheckFile(c.worldDir, paths, 'negative.ts', NEGATIVE);
  assert.notEqual(negative.code, 0, 'wrong variant value is rejected at the direct call');
  assert.ok(
