...
describe('system runtime heartbeat and class mapping', () => {
  ...
  it('emits bijective runtime class mappings matching stylesheet classes', () => {
    ...
    expect(classes['mt:2r'] ?? classes['marginTop:2r']).toBe('mt_2r')
    expect(classes['bg:blue.600'] ?? classes['background:blue.600']).toBe('bg_blue.600')
    expect(
      classes['hover:color:red.500'] ??
      classes['_hover:color:red.500'] ??
      classes['hover:c:red.500']
    ).toBe('hover:c_red.500')
