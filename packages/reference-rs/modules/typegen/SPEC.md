...
Public types are Reference UI-owned declarations. Core `SystemStyleObject` is assembled from `csstype.Properties` plus canon aliases; the packager wraps `BaseSystemStyleObject` when `strict` is set. `strict` is a printer option on `emit_dts_with`, not a spec field and not a `ui.config.ts` parse.
