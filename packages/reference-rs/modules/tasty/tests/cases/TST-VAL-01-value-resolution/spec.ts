...
async function verifyIntentAndSizeValues(api: TastyApi): Promise<void> {
  ...
  expect(intentKeyRaw.resolved?.types?.map(item => item.value)).toEqual([
    "'primary'",
    "'danger'",
  ])
  ...
  expect(intentValueRaw.resolved?.types?.map(item => item.value)).toEqual([
    "'blue'",
    "'red'",
  ])
  ...
  expect(sizeValueRaw.resolved?.types?.map(item => item.value)).toEqual([
    "'sm'",
    "'md'",
    "'lg'",
  ])
...
async function verifyToneAndVariantValues(api: TastyApi): Promise<void> {
  ...
  const concreteVariantMetaRaw = concreteVariantMeta.getUnderlyingType()?.getRaw() as {
    ...
    resolved?: {
      kind?: string
      types?: Array<{ kind?: string; members?: Array<{ name?: string }> }>
    }
  }
