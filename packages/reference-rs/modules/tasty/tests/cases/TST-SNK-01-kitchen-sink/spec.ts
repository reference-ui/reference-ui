...
async function verifyKitchenSinkAliases(api: TastyApi): Promise<void> {
  ...
  const cond = variantMeta.getUnderlyingType()?.getRaw() as {
    kind?: string
  ...
  const inst = buttonVariantMeta.getUnderlyingType()?.getRaw() as {
    kind?: string
    checkType?: { name?: string }
  }
...
async function verifyKitchenSinkButtonProps(api: TastyApi): Promise<void> {
  ...
  expect(toneLabelsMember?.getReferencedSymbol()?.getName()).toBe(
    'DocsReferenceToneLabels'
  )
...
async function verifyKitchenSinkInheritance(api: TastyApi): Promise<void> {
  ...
  expect(flattened.map(m => m.getName())).toEqual(
    expect.arrayContaining(['hasMenu', 'label', 'announceLabel'])
  )
