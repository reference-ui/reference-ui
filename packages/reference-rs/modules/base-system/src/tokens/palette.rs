//! Frozen `@reference-ui/lib` palette scales copied from `core/theme/colors.ts`.
//! Each hue is eleven shade steps (`50`–`950`) with a single `value` OKLCH string.
//! Light and dark are the same for these leaves; semantic `text.*` tokens live in `semantic`.
//! `fill` writes `colors.{hue}.{step}` entries and `--colors-{hue}-{step}` custom properties.

use super::{TokenDictionary, TokenLeaf};
...
/// Insert every palette shade into the token dictionary under the `colors` category.
pub fn fill(dict: &mut TokenDictionary) {
    for (hue, values) in PALETTES {
        for (step, value) in STEPS.iter().zip(values.iter()) {
            let path = format!("{hue}.{step}");
