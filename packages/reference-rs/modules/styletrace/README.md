...
The module is divided into three core phases:

1. **Resolution Pipeline**: Located in the resolver domain, this phase handles Oxc-driven type expansion. It traces type definitions for `StyleProps` and related aliases across the module graph to establish the ground-truth set of Reference style properties.
2. **Analysis Pass**: The analysis domain builds upon the resolved types to trace JSX wrapper components. It inspects component props and forwarding behavior to identify which wrappers maintain an unbroken connection to Reference primitives.
3. **Semantic Verification**: The testing strategy relies heavily on Rust tests for semantic coverage, isolating parsing and resolution logic from integration concerns. These tests are grouped by architectural concern rather than mapping one-to-one with source files.
