//! Provides shared string manipulation and pattern matching utilities for styletrace analysis parsing.
//! It takes various AST nodes like JSX identifiers, member expressions, and attribute names.
//! Evaluates and formats these elements to extract canonical names and paths.
//! Emits standardized string representations used consistently across the analysis models.
...
fn split_top_level(input: &str) -> Vec<String> {
    ...
    for ch in input.chars() {
        process_char(ch, &mut current, &mut depth, &mut parts);
    }
...
fn process_char(ch: char, current: &mut String, depth: &mut i32, parts: &mut Vec<String>) {
    match ch {
        '{' | '[' | '(' => {
            *depth += 1;
            current.push(ch);
