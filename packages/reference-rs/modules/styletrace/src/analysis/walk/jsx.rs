...
use oxc_ast::ast::{
    Argument, Expression, JSXAttributeItem, JSXAttributeValue, JSXChild,
...
pub fn collect_edges_from_jsx_child(child: &JSXChild<'_>, ctx: &mut WalkContext<'_>) {
    match child {
        ...
        JSXChild::ExpressionContainer(container) => collect_edges_from_jsx_expression(&container.expression, ctx),
        JSXChild::Spread(spread) => collect_edges_from_expression(&spread.expression, ctx),
    }
}

pub fn collect_edges_from_jsx_expression(expression: &JSXExpression<'_>, ctx: &mut WalkContext<'_>) {
    match expression {
...
pub fn collect_edges_from_jsx_element(element: &oxc_ast::ast::JSXElement<'_>, ctx: &mut WalkContext<'_>) {
    if let Some(target) = jsx_target(&element.opening_element.name, ctx.imports, ctx.primitive_names) {
...
fn walk_jsx_attribute(attribute: &JSXAttributeItem<'_>, ctx: &mut WalkContext<'_>) {
    match attribute {
        ...
        JSXAttributeItem::SpreadAttribute(spread) => collect_edges_from_expression(&spread.argument, ctx),
    }
}
...
pub fn is_react_create_element_call(call: &oxc_allocator::Box<'_, oxc_ast::ast::CallExpression<'_>>) -> bool {
    match &call.callee {
                ...
                match value {
                    JSXAttributeValue::ExpressionContainer(container) => {
                        match &container.expression {
                            ...
                            _ => bindings.expression_reads_style_prop(container.expression.to_expression()),
                        }
                    }
