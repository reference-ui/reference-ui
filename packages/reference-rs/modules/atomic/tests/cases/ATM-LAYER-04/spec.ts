...
const spec: AtomicCaseSpec = {
  ...
  verify(result) {
    ...
    expect(sheet.startsWith(LAYER_PREAMBLE)).toBe(true)
    ...
    expect(body).toContain('.mt_2r')
    expect(body).toContain('@container')
    expect(sheet.slice(0, utilitiesOpen)).not.toContain('.mt_2r')
    expect(sheet.includes('.mt_2r {') && !body.includes('.mt_2r')).toBe(false)
  },
}
