...
const spec: AtomicCaseSpec = {
  ...
  verify(result) {
    ...
    expect(recipes).toContain('button_v_primary')
    expect(recipes).toContain('color: white')
    expect(utilities).toContain('.\\@reference-ui\\/lib__mt_2r')
    expect(utilities).toContain('.\\@reference-ui\\/lib__bg_red\\.500')
    expect(utilities).not.toContain('button__base')
    expect(utilities).not.toContain('button_v_primary')
