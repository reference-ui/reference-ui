...
const spec: AtomicCaseSpec = {
  ...
  verify(result) {
    ...
    expect(result.stylesheet).toContain(classSelector(slash))
    expect(result.stylesheet).toContain(classSelector(token))
    expect(result.stylesheet).toContain(classSelector(hover))
    ...
    expect(result.stylesheet).toContain(classSelector(bracket as string))
  },
}
