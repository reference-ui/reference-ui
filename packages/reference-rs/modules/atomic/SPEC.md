...
| `LEAF` | AST leaf literal extraction & branch flattening | 11 | 11 | 0 |
...
- [x] `ATM-GHOST-02` `[reference]` `[seam]` —
  **Constructed names must be deterministic and equal the plan declarations.**
  Station `ATM-GHOST-02` still pins the compile-internal dictionary (`css.classes` keys → the selectors printed in the stylesheet). The runtime namer's constructions are held byte-equal to the plan declarations, slot and className in order, by the differential gate (`NEO-NAMER-01`), whose carve-outs are the token-refusal rule — a namer-side surplus is allowed only when its stem is a token-refusal shape — a braced (`{…}`) pair or a braceless brace-refusal tail matching the oracle's starts-with-`{`-and-ends-with-`}` refusal predicate — and its class is absent from the emitted sheet (witness `ATM-TOKEN-12`, braceless witness `{}}` on its border trio) — and the integer-key substrate carve: V8 ascends 2+ integer-like per-prop keys at object creation, so those members compare as sets while every member's declarations stay order-pinned (witness `ATM-NAME-08` intOrder; doom-5 key-order report, `.agents/doom/logs/`, 2026-09-20).
...
- [x] `ATM-LEAF-09` `[reference]` `[seam]` —
  **Authored `!` / `!important` suffixes on string literals must set `Want.important` and emit `mt_2r!`.**
  Station `ATM-LEAF-09`. `mt="2r!"` / `css({ p: '1r!' })` set `Want.important` and print `.mt_2r\!`. Narrow exception: refused responsive-object leaves (`ATM-LEAF-11`) warn and push no want instead — keeping the orphan important want would collide the unimportant served class under one `css.classes` key and break the injectivity gauge, so skipping is the cleaner refusal.
...
- [x] `ATM-LEAF-11` `[reference]` `[seam]` —
  **`!` on responsive-object leaves must refuse at the extraction boundary with a default diagnostic naming prop + leaf; the refused leaf pushes no want.**
  Station `ATM-LEAF-11`. Compile `css({ width: { base: '50px!', md: '60px' }, color: 'red' })`. Assert exactly one `ATM-W-RESPONSIVE-LEAF-IMPORTANT` default warning naming prop `width` and leaf `base`, no important `width` want, sibling leaf (`md`) and sibling prop (`color`) still extracting, the plan serving the plain `__w_50px` class as a diagnosed non-important fallback, and no orphan `!` rule in the sheet. Reason: the plan/query grammar is a five-tuple `(system, when, prop, value, important)` carrying ONE bool per plan (`runtime/serializer.rs`, neo `plans.ts`), and the builder applies that one flag to EVERY member of a responsive object (`runtime/builder.rs` `resolve_array`/`resolve_object`) — per-leaf importance has no carrier, so honoring it would need per-member plans/queries (a redesign of plan granularity, key grammar, S2 analysis, and merge). Detection mirrors plan capture (`ast_to_json_value`), so it fires exactly where the capture would drop the flag: string and template leaves plus transparent wrappers. Branching/dynamic leaves keep their proof-level diagnosis (the responsive-array `ATM-W-MISSING-STYLE-PLAN` precedent); const-spread responsive `!` already misses loudly at runtime and needs no refusal.
...
- [x] `ATM-RECIPE-02` `[reference]` `[seam]` —
  **Compiler must emit an authoritative variant lookup table for the runtime `recipe()` helper.**
  Compile a recipe with multiple variants and compound variants. Assert that `CompileResult` outputs a JSON variant table carrying the derivation inputs (qualified stem, per-axis value-name lists, defaults, compounds) plus the artifact-hoisted breakpoint list the runtime helper composes into class names directly without re-evaluating styles; nothing ships pre-composed.
...
- [x] `ATM-RECIPE-07` `[reference]` `[seam]` —
  **Responsive variant values must lower to `@container`-wrapped per-breakpoint classes with runtime table entries.**
  Compile `recipe({ className: 'buttonStyle', variants: { variant: { solid, outline } } })` where each value carries `_hover`/`_disabled` leaves. Assert each value also emits `{breakpoint}:`-prefixed classes (one per width breakpoint, after all plain rules) wrapped in that breakpoint's `@container (min-width: …)` query inside `@layer recipes`, with the hover/disabled descendants inside the query block. Assert the artifact-hoisted `responsiveBreakpoints` names the width breakpoints so a runtime `{ base: 'solid', md: 'outline' }` selection derives `md:`-prefixed classes from the per-axis value-name lists and emits both classes. Never `@media screen` (D8).
...
- [x] `ATM-DIAG-02` `[reference]` `[seam]` —
  **Dynamic non-literal expressions must emit fail-closed diagnostic warnings with source locations.**
  Station `ATM-DIAG-02`. Unresolvable identifiers get `warning` + file path; CSS still compiles. Since Error Correct Slice 5 these refusal/harvest lines ride the opt-in compiler channel (`logs: ['compiler']`) with locations intact; the default is silent (re-pointed in Slice 6).
...
- [x] `ATM-DIAG-06` `[reference]` `[seam]` — **[Error Correct S1]**
  **Non-ASCII source and selectors must compile without panic, and columns must be counted in UTF-16 code units.**
  Station `ATM-DIAG-06`. An emoji before a style prop pins the refusal at 3:61 (UTF-16 61, scalar 60, bytes 63) on the opt-in compiler channel since Slice 5 (re-pointed in Slice 6); the CJK/accented global selector paints; the unknown `_bogus` global condition carries its fragment source (`input/baseSystem.json:1:1`) on the default channel (global-surface lines stay default per the S6 adjudication). `line_col` (`diagnostics/site.rs`) counts UTF-16 units with unit tests; the global walker threads the fragment source since this slice. Rust byte offsets and editor columns disagree for any non-ASCII file, which would make every diagnostic position wrong past the first multibyte character.
- [x] `ATM-DIAG-07` `[reference]` `[seam]` — **[Error Correct S5]**
...
- [x] `ATM-DIAG-11` `[reference]` `[seam]` — **[Error Correct S2/S5]**
...
- [x] `ATM-DIAG-12` `[reference]` `[seam]` — **[Error Correct S2]**
...
- [x] `ATM-DIAG-14` `[reference]` `[seam]` — **[Error Correct S2]**
...
| `ATM-SITE-78` | `[x]` | `[seam]` | `tests/cases/ATM-SITE-78/` |
| `ATM-SITE-79` | `[x]` | `[seam]` | `tests/cases/ATM-SITE-79/` |
| `ATM-SITE-80` | `[x]` | `[seam]` | `tests/cases/ATM-SITE-80/` |
| `ATM-SITE-81` | `[x]` | `[seam]` | `tests/cases/ATM-SITE-81/` |
...
| `ATM-LEAF-11` | `[x]` | `[seam]` | `tests/cases/ATM-LEAF-11/` |
