 ...
 * Emits zero-allocation static Rust modules into system/src/canon.
 */
...
const systemCrateDir = path.resolve(__dirname, '../src/canon');
...
function emitAllModules(dialect: DialectData, targetDir: string): void {
  ...
  fs.writeFileSync(path.join(targetDir, 'mod.rs'), emitModRs(), 'utf-8');
...
async function main(): Promise<void> {
  ...
  emitAllModules(dialect, systemCrateDir);
  console.log('[canon] Successfully generated system/src/canon/*.rs');
}
