/**
 * Unit test suite for native package publishing and manifest staging.
 * Asserts correct assembly of platform packages and temporary root modifications.
 * Validates npm provenance flags and clean restoration of root package.json.
 */
...
async function importPublishNativeModule(options: {
  ...
  vi.doMock('../shared/paths', () => ({
    ...
    distDir: resolve(options.packageDir, 'dist'),
    artifactsDir: options.artifactsDir,
    npmDir: resolve(options.packageDir, 'dist', 'npm'),
...
function createNativePublishFixture() {
  ...
  const artifactsDir = resolve(packageDir, 'dist', 'artifacts')
  const npmDir = resolve(packageDir, 'dist', 'npm')
