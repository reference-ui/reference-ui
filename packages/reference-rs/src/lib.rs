...
#[cfg(feature = "napi")]
#[napi]
pub fn analyze_styletrace(root_dir: String, workspace_hint: Option<String>) -> Result<String> {
    let normalized_root = PathBuf::from(&root_dir);
    let workspace_hint = workspace_hint.as_deref().map(PathBuf::from);
    let result = trace_style_jsx_names_with_hint(&normalized_root, workspace_hint.as_deref())
