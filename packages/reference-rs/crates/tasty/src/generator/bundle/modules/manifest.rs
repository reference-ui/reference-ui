...
fn should_warn_duplicate_symbol_name(
    symbol_ids: &[String],
    symbols_by_id: &BTreeMap<String, TastySymbolIndexEntry>,
    symbols: &BTreeMap<String, crate::model::TsSymbol>,
...
fn type_parameters_equivalent(
    left: &[crate::model::TsTypeParameter],
    right: &[crate::model::TsTypeParameter],
) -> bool {
    left.len() == right.len()
...
fn optional_type_ref_equivalent(
    left: &Option<crate::model::TypeRef>,
    right: &Option<crate::model::TypeRef>,
) -> bool {
    match (left, right) {
...
fn optional_boxed_type_ref_equivalent(
    left: &Option<Box<crate::model::TypeRef>>,
    right: &Option<Box<crate::model::TypeRef>>,
) -> bool {
    match (left, right) {
        (Some(left), Some(right)) => type_ref_equivalent(left, right),
        (None, None) => true,
...
fn tuple_elements_equivalent(
    left: &[crate::model::TupleElement],
    right: &[crate::model::TupleElement],
) -> bool {
    left.len() == right.len()
        && left.iter().zip(right).all(|(left, right)| {
...
fn template_literal_parts_equivalent(
    left: &[crate::model::TemplateLiteralPart],
    right: &[crate::model::TemplateLiteralPart],
) -> bool {
    use crate::model::TemplateLiteralPart;
