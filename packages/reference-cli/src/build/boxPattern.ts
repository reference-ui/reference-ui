...
import { createBoxPatternCollector } from '../system/collectors/extendPattern.js'
import type { BoxPatternExtension } from '../system/collectors/extendPattern.js'
import { COLLECTOR_KEY } from '../system/collectors/extendPandaConfig.js'
...
function generateBoxPattern(extensions: CollectedExtension[]): string {
  ...
  // Push directly to globalThis — no import, so it works when panda.config runs in app context.
  return `/** Generated by build/boxPattern.ts - do not edit manually */

;(globalThis['${COLLECTOR_KEY}'] = globalThis['${COLLECTOR_KEY}'] || []).push({
  patterns: {
    extend: {
      box: {
        properties: {
