...
2. StyleProps are Static CSS Classes: StyleProps compile directly into atomic CSS classes at build time. No Tailwind or arbitrary CSS classes.
...
6. The Dynamic Styling Fine Line: Known ternaries (e.g. color={active ? 'primary' : 'muted'}) are statically pre-compiled. But calculated runtime math (e.g. dynamic zIndex, opacity formulas) is dropped by static codegen and must use inline style={{ ... }}.
`
