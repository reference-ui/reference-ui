...
test.describe('Popover Composition Gates & Browser Proofs', () => {
  test('PO-DOM-01 & PO-DOM-02: Toggles Popover open/closed and updates ARIA attributes', async ({
    ...
    await page.waitForTimeout(300)
    await snap(page, 'basic-open')

    await page.getByTestId('btn-popover-close').click()
    await expect(trigger).toHaveAttribute('aria-expanded', 'false')
    await expect(content).toHaveCount(0)
    await page.waitForTimeout(300)
    await snap(page, 'basic-closed')
  })
  ...
  test('Outside press light-dismisses the popover', async ({ mount, page }) => {
    await mount('components/Popover/Popover/Basic')
    const trigger = page.getByTestId('btn-popover-trigger')
    const content = page.getByTestId('popover-content')
    ...
    await expect(content).toHaveCount(0)
    await page.waitForTimeout(300)
    await snap(page, 'outside-dismissed')
  })

  test('PO-FLIP-01: Popover should flip to the opposite side when its preferred side overflows', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/Placements')
    ...
    await page.waitForTimeout(300)
    await snap(page, 'flip-top')
  })

  test('PO-SHIFT-01: Popover should shift within collision padding when Content partially overflows', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/Placements')
    ...
    await page.waitForTimeout(300)
    await snap(page, 'shift-padding')
  })

  test('PO-ARROW-01: Popover should center Arrow on its anchor', async ({ mount, page }) => {
    await mount('components/Popover/Popover/ClickToOpen')
    ...
    await page.waitForTimeout(300)
    await snap(page, 'arrow-centered')
  })

  test('PO-HOVER-01: Popover should issue delayed open request when mouse remains over openOnHover Trigger', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverCard')
  ...
  test('PO-HOVER-02: Popover should stay open when the pointer travels diagonally from Trigger toward interactive Content', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
    ...
    await expect(content).toBeVisible()
    await expect(log).toHaveText('open')
    await snap(page, 'hover-grace-open')
  })

  test('PO-HOVER-03: Popover should request delayed close when the pointer leaves the safe region and should cancel it on reentry', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-HOVER-04: Popover should preserve one open interaction when the pointer moves from Content back to Trigger through their padded gap', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-HOVER-05: Popover should leave hover grace when pointer travel is slow, reversed, or crosses the side opposite Content', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-HOVER-07: Popover should remain open when Trigger is clicked within the 300-millisecond impatient window after hover-open', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-HOVER-08: Popover should use normal Trigger dismissal when a deliberate click occurs after the patient threshold', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-HOVER-09: Popover should avoid mouse-intent timers when touch or non-hover pen input synthesizes pointer entry', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-HOVER-10: Popover should stay open when pointer or focus remains in interactive hover Content', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-HOVER-11: Popover should share one open intent when keyboard focus drives an openOnHover instance', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/HoverGrace')
  ...
  test('PO-LAYER-01: Popover should register once when it is a child layer of Overlay', async ({
    mount,
    page,
  }) => {
    await mount('components/Popover/Popover/NestedLayer')
    ...
    await page.waitForTimeout(300)
    await snap(page, 'nested-dialog-open')

    await popoverTrigger.click()
    await expect(popover).toBeVisible()
    await expect(count).toHaveText('2')
    await page.waitForTimeout(300)
    await snap(page, 'nested-popover-open')
