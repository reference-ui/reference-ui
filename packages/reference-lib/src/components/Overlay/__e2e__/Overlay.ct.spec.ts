...
test.describe('Overlay Deep SPEC & Production Verification Suite', () => {
  ...
  test.describe('1. DOM & Anatomy Contracts', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Dialog')
    ...
    test('OV-DOM-01 & OV-DOM-05: Renders no portal content initially, renders Backdrop and Content upon open', async ({
      ...
      await snap(page, 'dialog-basic-open')
    ...
    test('OV-DOM-08 & OV-DOM-09: Duplicate parts fail atomically and emit diagnostic warnings', async ({ mount, page, }) => {
  ...
  test.describe('2. Escape Ordering & Cancellation', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Nested')
    ...
    test('OV-ESC-03: Observes latest state closure and prevents escape dismissal after rerender', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
    ...
    test('OV-ESC-06: Non-dismissible AlertDialog when application prevents Escape', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
  ...
  test.describe('3. Layer Stacks & Nesting Interactions', () => {
    ...
    test('OV-LAYER-08: Overlay should retain a valid stack when layers are removed out of registration order', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Nested')
      await expect(page.getByTestId('nested-fixture-root')).toBeVisible()
  ...
  test.describe('4. Outside Press & Pointer Mechanics', () => {
    ...
    test('OV-OUT-11: Overlay should retain cancelable geometric outside dismissal when no Backdrop part is authored', async ({
      ...
      await page.evaluate(() => {
        const target = document.elementFromPoint(400, 400) || document.documentElement
        const opts = {
          bubbles: true,
          cancelable: true,
          clientX: 400,
    ...
    test('OV-ISO-04: Overlay should defer outside-press only while inert isolation is on', async ({ mount, page, }) => {
      ...
      await page.evaluate(() => {
        const target = document.elementFromPoint(450, 450) || document.documentElement
        const opts = {
          bubbles: true,
          cancelable: true,
          clientX: 450,
  ...
  test.describe('5. Scroll Lock Mechanics', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Scroll')
    ...
    test('OV-SCROLL-02: Overlay avoids layout shift when locking page with scrollbar and authored root styles', async ({ mount, page, }) => {
  ...
  test.describe('6. Inert Background Isolation', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Inert')
    ...
    test('OV-INERT-05: Live regions and toast host are exempted from inerting', async ({ mount, page, }) => {
      await page.getByTestId('btn-open-inert-dialog').click()
      await expect(page.getByTestId('inert-content')).toBeVisible()
  ...
  test.describe('7. Edge Sheets & Handle Gestures', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Edge')
    ...
    test('OV-EDGE-04: Overlay publishes nested edge stack CSS variables (--reference-overlay-index, --reference-overlay-count)', async ({ mount, page, }) => {
  ...
  test.describe('8. Themed Portal Scoping', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Theme')
  ...
  test.describe('Focus & Restoration Contracts (OV-FOCUS-* & OV-RESTORE-*)', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Focus')
  ...
  test.describe('Anchored Floating UI & Arrow Contracts (OV-POS-*)', () => {
    test.beforeEach(async ({ mount, page }) => {
      await mount('components/Overlay/Overlay/Anchor')
  ...
  test.describe('10. Presence & Exit Lifecycle', () => {
    test('OV-PRES-01: Keeps both Backdrop and Content mounted with data-state="closed" during owned CSS exit', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
    ...
    test('OV-PRES-02: Waits for slower part (Backdrop 250ms, Content 100ms)', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
    ...
    test('OV-PRES-02: Waits for slower part (Backdrop 100ms, Content 250ms)', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
    ...
    test('OV-PRES-04: Completes close immediately with zero/no-animation styles or reduced motion', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
    ...
    test('OV-PRES-03: Ignores bubbled child animation events when Content itself is still exiting', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
    ...
    test('OV-PRES-05: Maintains one current lifecycle during rapid close -> reopen -> close sequence', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Dialog')
  ...
  test.describe('11. Modal Pointer Isolation', () => {
    ...
    test('OV-RESTORE-05: Cancels pending focus restoration when reopened during exit animation', async ({ mount, page, }) => {
      const openBtn = page.getByTestId('btn-open-reopen-pointer')
      const content = page.getByTestId('dialog-reopen-pointer-content')
      const quickCycleBtn = page.getByTestId('btn-reopen-pointer-quick-cycle')
      const finalCloseBtn = page.getByTestId('btn-close-reopen-pointer-final')
  ...
  test.describe('12. Trigger Interaction Suite (OV-TRG-03, OV-TRG-04, OV-TRG-05)', () => {
    ...
    test('OV-TRG-05: Bridges Tab from Trigger into Content when isolation focus is off, advances past Trigger and dismisses on exit', async ({ mount, page, }) => {
  ...
  test.describe('OV-ESC-07: Native top-layer popover Escape', () => {
    ...
    test('OV-ESC-07: Overlay waits to handle Escape when a native top-layer popover consumes the first key', async ({ mount, page, }) => {
  ...
  test.describe('OV-INERT-04 / OV-INERT-07: Dynamic inert and restore', () => {
    ...
    test('OV-INERT-07: Overlay restores background attributes once when animated exit completes and cancels that restoration when it reopens', async ({ mount, page, }) => {
  ...
  test.describe('OV-SCROLL-04: Portalled shard scrolling', () => {
    ...
    test('OV-SCROLL-04: Overlay applies the same edge-aware scrolling rules when the scrollable is in a registered portalled shard', async ({ mount, page, }) => {
  ...
  test.describe('OV-RESTORE-06 / OV-FOCUS-08: Focus robustness', () => {
    test('OV-RESTORE-06: Overlay restores only the outer origin when closing a parent cascades through nested layers', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Nested')
      await expect(page.getByTestId('nested-fixture-root')).toBeVisible()
    ...
    test('OV-FOCUS-08: Overlay resolves initial focus after Content mounts when its public ref and internal FocusLock ref are composed', async ({ mount, page, }) => {
      await mount('components/Overlay/Overlay/Focus')
      await expect(page.getByTestId('focus-fixture-root')).toBeVisible()
  ...
  test.describe('OV-INERT-03 / OV-SCROLL-05: Out of order exit', () => {
    ...
    test('Keep background hidden and document locked until the last modal finishes its exit', async ({ mount, page }) => {
      ...
      await page.getByTestId('btn-ooo-close-1').evaluate((el: HTMLElement) => el.click())
      await page.getByTestId('btn-ooo-close-2').evaluate((el: HTMLElement) => el.click())
  ...
  test.describe('OV-SCROLL-10: Interrupted teardown', () => {
    ...
    test('Clean scroll-lock effects exactly once if teardown is interrupted or reopened during exit', async ({ mount, page }) => {
      // 1. Open
  ...
  test.describe('OV-ENV-04: StrictMode resilience', () => {
    ...
    test('One modal side effect even when React replays lifecycles', async ({ page }) => {
      ...
      // 2. Close
      await page.getByTestId('btn-strict-toggle').evaluate((el: HTMLElement) => el.click())
