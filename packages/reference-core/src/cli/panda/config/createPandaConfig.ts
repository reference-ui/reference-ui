...
import { createFontSystem } from '../fontFace'
...
export async function createPandaConfig(coreDir: string): Promise<void> {
  try {
    ...
    // Step 1: Generate font system (font() -> font.ts with tokens, @font-face, recipe, pattern)
    console.log('[createPandaConfig] Generating font system...')
    await createFontSystem(coreDir)

    // Step 2: Generate box pattern (extendBoxPattern -> inlined box.ts)
    console.log('[createPandaConfig] Generating box pattern...')
    await createBoxPattern(coreDir)

    // Step 3: Find config files (exclude box extension files; they're in box.ts)
    ...
    // Step 4: Create entry file from template
    ...
    // Step 5: Bundle with esbuild (reusable helper, in-memory output)
    ...
    // Step 6: Write final panda.config.ts
