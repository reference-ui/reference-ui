...
- **`runner.ts`** – Runs each matched file via `microbundle`, with a global collector set so that when the file calls `extendPandaConfig(partial)`, the partial is captured. Returns all collected fragments.
