...
### 1.3 `packager/packages.ts` — Maintainability Index: 80 (lowest in codebase)

**Issue**: Dense, hard-to-read package definitions.
...
export const PACKAGES: PackageDefinition[] = [
  {
    ...
    /** Entry point for React primitive components and hooks */
    entry: 'src/styled/index.ts',
    /** Include types, patterns, utilities */
    include: ['src/styled/**', 'src/patterns/**', 'src/utilities/**'],
    /** Omit internal test fixtures */
...
function validatePackageDefinition(pkg: PackageDefinition): void
```

**Impact**: Maintainability 80 → 100+

**Files**: `packager/packages.ts`  
**Estimate**: 30 minutes
...
### 1.5 `lib/child-process.ts` — Physical LOC: 219, large function

**Issue**: `spawnMonitoredAsync` is 82 LOC with event handling + monitoring logic.

**Action**:

```typescript
// Extract:
- attachMonitoring(child: ChildProcess, options: MonitorOptions): MonitorResult
- collectChildOutput(child: ChildProcess): Promise<{ stdout: string, stderr: string }>
- waitForExit(child: ChildProcess): Promise<ExitInfo>

// Compose in spawnMonitoredAsync
```

**Impact**: Easier to test, maintain

**Files**: `lib/child-process.ts`  
**Estimate**: 45 minutes

---

**Phase 1 Total**: ~4.5 hours  
**Result**: All high-complexity issues resolved

---

## Phase 2: Architectural Improvements

### 2.1 Self-Building CLI (from `build-plan.md`)

**Issue**: CLI requires external `build-cli.mjs` script; can't rebuild itself.

**Goal**: Make CLI portable by having packager bundle the CLI as part of `ref sync`.

**Action**:

```typescript
// In packager/bundler.ts:
export async function bundleAllPackages(coreDir: string) {
  // NEW: Bundle CLI first (dist/cli/)
  await bundleCLI(coreDir)

  // Then bundle packages (node_modules/@reference-ui/*)
  await bundlePackages(coreDir)
}

// New function:
async function bundleCLI(coreDir: string): Promise<void> {
  // Bundle src/cli → dist/cli/index.mjs + workers
  // Apply shebang + chmod 755
}
```

**Benefits**:

- Single entry point (`ref sync` becomes self-sufficient)
- CLI dogfoods its own bundler
- Eliminates build script dependency

**Files**: `packager/bundler.ts`, `packager/cli-bundle.ts` (new)  
**Estimate**: 2 hours

**Status**: Documented in `build-plan.md`, ready to implement

---

### 2.2 Public Runtime API (from `todo.md`)

**Issue**: `tokens()`, `keyframes()`, `font()` need to be eval-compatible.

**Goal**: System eval monitors virtual filesystem and provides public API.

**Action**:

```typescript
// In system/eval/index.ts:
export { tokens, keyframes, font } from '@reference-ui/core/runtime'

// Extend eval to watch virtual:fs:change and re-scan for API calls
```

**Benefits**: Users can call design system APIs at runtime

**Files**: `system/eval/index.ts`, `system/eval/scanner.ts`  
**Estimate**: 1.5 hours

**Status**: Planned in `todo.md`, spec TBD

---

### 2.3 Combine Scripts + CLI (from `todo.md`)

**Issue**: Unclear separation between `scripts/` and `cli/`.

**Goal**: Clarify responsibilities or merge.

**Action**: Audit current `scripts/` usage → decide if CLI should subsume it.

**Estimate**: 30 minutes analysis, 1 hour implementation (if merging)

---

**Phase 2 Total**: ~4 hours  
**Result**: CLI is portable, self-building, and runtime-aware
