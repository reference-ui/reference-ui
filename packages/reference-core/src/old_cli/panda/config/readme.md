...
- **Config builder** – `createPandaConfig` takes base config + collected fragments (from `globalThis[COLLECTOR_KEY]`), deep-merges, and writes `panda.config.ts`. Runs after eval in the sync flow.

## Config generation

- **`panda.base.ts`** – Base Panda config (source of truth). Exports `baseConfig` for programmatic merge.
- **`panda.config.ts`** – Generated by `createPandaConfig`. Re-exports from base when no fragments; otherwise merges base + fragments and exports `defineConfig(merged)`. Added to `.gitignore`.
- **`createPandaConfig(coreDir)`** – Reads fragments from `globalThis[COLLECTOR_KEY]`, merges with base, writes `panda.config.ts`. Invoked by sync after eval.
