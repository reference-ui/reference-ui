...
function createGeneratedWorkspaceFixture(): GeneratedWorkspaceFixture {
  ...
  return { workspaceDir, outDir, scopeDir }
}
...
function expectGeneratedOutputsRemoved(outDir: string, scopeDir: string): void {
...
function expectWorkspaceLinksIntact(scopeDir: string): void {
...
describe('clean/command', () => {
  it('removes generated outDir and generated package links', async () => {
    const { workspaceDir, outDir, scopeDir } = createGeneratedWorkspaceFixture()
    mockCleanCommandDependencies(outDir)
    ...
    expectGeneratedOutputsRemoved(outDir, scopeDir)
    expectWorkspaceLinksIntact(scopeDir)
  })
})
